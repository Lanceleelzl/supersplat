...
### 3.4 Web 开发专项流程 (Web Dev Workflow)
> 针对 Web 前端任务 (SuperSplat 核心)，遵循 Antigravity 级构建标准：

1.  **Foundation (地基)**: 优先确立 CSS 变量/Design Tokens (颜色、排版)，严禁硬编码颜色值。
2.  **Components (组件)**: 构建可复用的组件，保持单一职责。
3.  **Polish (打磨)**:
    - **SEO**: 确保 Title, Meta Description, Semantic HTML (`header`, `nav`, `main`) 正确。
    - **Performance**: 优化加载性能，避免布局抖动 (CLS)。

### 3.5 Workflow 自动化 (Agentic Workflows)
- 支持识别 `.agent/workflows/*.md` 定义的工作流。
- 如果遇到 `// turbo` 标记，自动执行对应的命令步骤。

## 4. 后端与算法服务 (Backend & Algorithm Services)

> **核心目标**: 为前端提供强大的计算支持。无论使用何种语言，必须确保**易部署、易调用、高性能**。

### 4.1 技术选型策略 (Tech Stack Strategy)
- **Python**: 首选用于快速原型、AI 模型推理 (PyTorch/TensorFlow)、数据处理胶水层。
- **C++**: 首选用于高频几何计算、大规模点云处理、实时渲染算法。**AI 负责扫除所有 C++ 障碍。**
- **Rust/Go**: 可选的高性能替代方案（内存安全/并发优势）。

### 4.2 🧱 语言盲区辅助协议 (Language Gap Protocol)
> **适用场景**: 当使用主人不熟悉的高性能语言（C++）或不精通的语言（Python）时，必须执行以下“保姆级”标准：

1.  **服务化封装 (Service Encapsulation)**:
    - **黑盒交付**: 后端必须被封装为**独立运行的服务** (Web Server)。
    - **一键启动**: 必须提供 `start_server.bat` (Windows)，自动处理环境激活、依赖检查和服务启动。主人只需双击。
    - **严禁**要求主人手动输入复杂的命令行参数。
2.  **Python/C++ 混合策略**:
    - **C++**: 负责核心算法（Ply 解析、KD-Tree/Octree 构建、体素化计算、距离场生成）。
    - **Python**: 负责 Web 接口 (FastAPI) 和胶水逻辑。
    - **绑定**: AI 负责编写 C++ binding (pybind11)，主人只需关注 Python 层的 API 定义。
3.  **保姆级注释**: 核心算法代码必须包含逻辑图解或详细中文注释。

### 4.3 👁️ 算法可视化与数据流 (Algo-Vis & Data Pipeline)
> **针对无人机航线规划/安全距离计算场景**:

1.  **所见即所得 (Visual Verification)**:
    - 后端计算出的 **体素 (Voxel)**、**安全包围盒 (Safe Bounding Box)** 或 **路径 (Path)**，**必须**能在前端 SuperSplat 中渲染出来。
    - 前端需开发对应的 `DebugLayer` 或 `VoxelLayer` 来展示后端结果，验证算法准确性。
2.  **高效数据流**:
    - **Ply 数据**: 前端仅传递文件路径（如果本地同源）或使用二进制流上传。
    - **结果数据**: 体素数据量大，优先使用压缩二进制 (Binary) 或紧凑 JSON 传输，前端解析后利用 `Instancing` 技术高效渲染。
